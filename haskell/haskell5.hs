...
ordenar (x:y:[]) | x<=y =[x]++[y]
                 | x>y =[y]++[x]
ordenar (x:xs) = ordenar (quitar (maximo xs) (x:xs)) ++ [maximo xs]
...
aplanar :: [[Char]] -> [Char]
aplanar (x:xs) = palabraChar (x:xs)

palabraChar :: [[Char]] -> [Char]
palabraChar (x:xs) | principiooo (x:xs) ==" " = []
                   | otherwise = x ++ palabraChar (xs)  
palabraChar (xs) = []     
...
--f)
aplanarConBlanco :: [[Char]] -> [Char]
aplanarConBlanco (x:xs) = palabraCharBlanco (x:xs)

palabraCharBlanco :: [[Char]] -> [Char]
palabraCharBlanco (x:xs) | principiooo (x:xs) ==" " = []
                         | otherwise = x ++ [' '] ++ palabraCharBlanco (xs)  
palabraCharBlanco (xs) = [] 


--g)
aplanarConBlancos :: [[Char]] -> Integer -> [Char]
aplanarConBlancos (x:xs) n = palabraCharBlancoN (x:xs) n


palabraCharBlancoN :: [[Char]] -> Integer -> [Char]
palabraCharBlancoN (x:xs) n | principiooo (x:xs) ==" " = []
                            | otherwise = x ++ sumarBlancos n ++ palabraCharBlancoN (xs) n 
palabraCharBlancoN (xs) n = []


sumarBlancos :: Integer -> [Char]
sumarBlancos n | n==1 = [' ']
               | n>1 = [' '] ++ sumarBlancos (n-1)
...
sumaAcumulada (x:y:[]) = [x] ++ [x + y]
sumaAcumulada (x:y:xs) = [x] ++ sumaAcumulada ((x+y):xs)

--5.2
menorDivisor :: Integer -> Integer
menorDivisor n = divideA n 2 
                

divideA :: Integer -> Integer -> Integer
divideA n i | mod n i ==0 =i
            | otherwise = divideA n (i+1)
...
noesPrimo n | menorDivisor n ==n =False
            | otherwise =True         

menorDivisorP :: Integer -> Integer
menorDivisorP n | noesPrimo n && (esPrimo (menorDivisor n)) = (menorDivisor n) 
                | otherwise =n

maximoDivisorP :: Integer -> Integer
maximoDivisorP n = divideAP n (n-1)     
 

divideAP :: Integer -> Integer -> Integer
divideAP n i | esPrimo n =n
             | noesPrimo n && mod n (i) ==0 && esPrimo i =i
             | otherwise = divideAP n (i-1)

dividirPrimos :: Integer -> [Integer]
dividirPrimos n | div n (maximoDivisorP n) ==1  =[maximoDivisorP n]
                | otherwise = ordenar ([maximoDivisorP n] ++ dividirPrimos ((div n (maximoDivisorP n))))

descomponerEnPrimos :: [Integer] -> [[Integer]]     
descomponerEnPrimos [x] | esPrimo x = [[x]] 
                        | noesPrimo x = [dividirPrimos x]
descomponerEnPrimos (x:xs) | esPrimo x = [[x]] ++ descomponerEnPrimos (xs)   
                           | noesPrimo x = [dividirPrimos x] ++ descomponerEnPrimos (xs)                        
